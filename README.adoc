...
----
Add roles-based access control to the 
...
The [hotspot=10-32]`SecureSystemClient` class inherits methods from the `SystemClient` class. These methods hand the HTTP `GET` request to the system service to retrieve system properties.
However, the [hotspot=10-32]`SecureSystemClient` class overrides the [hotspot=22-25]`buildClientBuilder()` method by adding the authorization header to the returned builder, [hotspot=24]`return builder.header(HttpHeaders.AUTHORIZATION, authHeader)`.
By adding the authorization header to the client request, the [hotspot=10-32]`SecureSystemClient` class can access services that are secured.
...
----
Create the `JWTResource` class to enable a 
service that retrieves information from the token.
...
The [hotspot=47-53]`getCustomClaim()` function retrieves the custom claim from the token if the authenticated user has the [hotspot=45 file=4]`admin` role. Otherwise, it returns a [hotspot=52]`Status.FORBIDDEN` message.

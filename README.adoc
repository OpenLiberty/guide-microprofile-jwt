...
:page-tags: ['MicroProfile JWT', "JSON Web Tokens", "MicroProfile", "microservices"]
:page-permalink: /guides/{projectid}
:page-related-guides: ['cdi-intro']
:common-includes: https://raw.githubusercontent.com/OpenLiberty/guides-common/master
:source-highlighter: prettify
= Securing microservices with MicroProfile JSON Web Token

Learn how to control user and role access to microservices with MicroProfile JSON Web Token (MicroProfile JWT).
...
For microservices, token based authentication mechanism offers a very lightweight way for security controls, and security tokens propagate user identities across different services. JSON Web Token (JWT) is becoming the most common token format because it follows a well defined and known standard.

MicroProfile JWT standard defines the required format of JWT for authentication and authorization, maps its token claims to various Java EE container APIs, and makes the set of claims available through getters.

You will add MicroProfile JWT to validate security tokens in the `system` and `inventory` microservices.  You will use token based authentication mechanism to authenticate, authorize and verify user identities based on a security token.

In addition, you will learn how to verify token claims through getters using MicroProfile JWT.
...
You are redirected to a page that displays the user who is logged in, the current OS of your localhost, and the security token that the front end uses when sending the HTTP request to access the data from the back end.
In addition, if you log in as an `admin` user, the current inventory size should appear. If you log in as a normal `user`, you see the message `You are not authorized to access the inventory service.` instead, which means you cannot access the inventory service in the back-end.
...
|===
| `issuer` | issues security tokens, should matches the `iss` claim in the JWT
| `audiences` | identifies the recipients, should match the `aud` claim in the JWT
| `keyName`| specifies the JWT signature validation key, should match the key in the truststore in the SSL configuration
|===
...
Role names used in `@RolesAllowed` are mapped to group names in the `groups` claim of the JWT, which results in an authorization decision wherever the security constraint is applied.
...
Create a `start/backendServices/src/main/java/io/openliberty/guides/inventory/JwtResource.java` file to enable a service that retrieves information from the token.
...
`getJwtUsername()` retrieves the user name from the injected `JsonWebToken jwtPrincipal` by calling `getName()`.
...
`getCustomClaim()` retrieves the custom claim from the token if the authenticated user has the `admin` role. Otherwise, it returns a `Status.FORBIDDEN` message.
`SecurityContext.isUserInRole(String)` returns `true` for any role that appears in the MicroProfile JWT `groups` claim because role names are mapped to group names in the claim.
...
When the Open Liberty application server has started, you can log in to the system with the simple front end. The entire front end code is provided for you to test if you can retrieve information from the back end services using a valid security token.
You cannot directly visit a back end URL from a browser because you don't have a valid token in the authorication header when you try to send HTTP requests via browser. You have to use the provided front end to create a web client to send HTTP GET requests with valid security tokens.
...
The following endpoint is used to log in to the application:

* `http://localhost:9090/application.jsf`

Once you log in, you can see some basic information about your current session, including the user name of the user who is currently logged in, your role[s], your current operating system, the size of your inventory service, and the JWT that is  used for authorization.
...
You will write `SystemEndpointTest`, `InventoryEndpointTest` and `JwtTest` classes to test if different services can be accessed with valid tokens.
...
You have learned how to use MicroProfile JWT to validate JWT and authorize users to secure your microservices.

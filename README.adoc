...
The MicroProfile JWT feature, [hotspot=3]`mpJwt`, has been added as a dependency in your `start/backendServices/pom.xml` file.
...
[role="code_command hotspot=1-23", subs="quotes"]
----
...
The [hotspot=22]`<mpJwt/>` element is required to configure the injection of the caller's JWT.

|===
| [hotspot=22]`keyName` | Specifies the key alias name to locate the public key for JWT signature validation and must be in the keystore in the SSL configuration.
| [hotspot=22]`audiences` | Identifies the recipients and must match the `aud` claim in the JWT.
| [hotspot=22]`issuer` | Issues security tokens and must match the `iss` claim in the JWT.
|===
...
=== Securing the system service
[role="code_command hotspot=13-20", subs="quotes"]
----
Add roles-based access control to the SystemResource class 
...
Role names that are used in the [hotspot=15 file=1]`@RolesAllowed` annotation are mapped to group names in the `groups` claim of the JWT, which results in an authorization decision wherever the security constraint is applied.

The [hotspot=15 file=1]`@RolesAllowed({"admin", "user"})` annotation allows only authenticated users with the role of `admin` or `user` to access the `system` service.
...
=== Securing the inventory service
[role="code_command hotspot=19-52", subs="quotes"]
----
Add roles-based access control to the `InventoryResource` class
...
Similarly, for each specific service, the [hotspot=47 file=2]`@RolesAllowed` annotation sets which roles are allowed to access it. Therefore, the inventory service is properly secured, as well.

In addition, the [hotspot=28-44 file=2]`getPropertiesForHost()` method gets the HTTP Authorization header, which contains the corresponding security token, from the HTTP request caller. The method uses this authorization header to retrieve information from the `system` service.

The final addition of the [hotspot=33 file=2]`manager.get(hostname, authHeader)` code adds the authentication header to the client that sends the HTTP `GET` request to the `system` service.
...
[role="code_command hotspot=10-32", subs="quotes"]
----
...
The [hotspot=10-32 file=3]`SecureSystemClient` class inherits methods from the `SystemClient` class. These methods hand the HTTP `GET` request to the system service to retrieve system properties.
However, the [hotspot=10-32 file=3]`SecureSystemClient` class overrides the [hotspot=22-25 file=3]`buildClientBuilder()` method by adding the authorization header to the returned builder, [hotspot=24 file=3]`return builder.header(HttpHeaders.AUTHORIZATION, authHeader)`.
By adding the authorization header to the client request, the [hotspot=10-32 file=3]`SecureSystemClient` class can access services that are secured.
...
[role="code_command hotspot=17-54", subs="quotes"]
----
Create the `JWTResource` class to enable a service that retrieves information from the token.
...
The [hotspot=27-29 file=4]`getJwtUsername()` function retrieves the user name from the injected `JsonWebToken jwtPrincipal` token by calling the [hotspot=28 file=4]`getName()` method.

The [hotspot=34-42 file=4]`getJwtGroups()` function retrieves the user groups information from the JSON Web Token.
The [hotspot=36 file=4]`SecurityContext.getUserPrincipal()` method returns an object in the `java.security.Principal` type. This object is also an instance of the `org.eclipse.microprofile.jwt.JsonWebToken` type, which has the [hotspot=39 file=4]`getGroups()` method.

The [hotspot=47-53 file=4]`getCustomClaim()` function retrieves the custom claim from the token if the authenticated user has the [hotspot=45 file=4]`admin` role. Otherwise, it returns a [hotspot=52 file=4]`Status.FORBIDDEN` message.
The [hotspot=48 file=4]`SecurityContext.isUserInRole(String)` method returns a `true` value for any role that appears in the MicroProfile JWT `groups` claim because role names are mapped to group names in the claim.
The [hotspot=49 file=4]`jwtPrincipal.getClaim("customClaim")` method retrieves the value of the custom claim by its name. This method is useful when you want to validate information about a custom claim from the security token.
...
[role="code_command hotspot=13-49" subs="quotes"]
----
...
[role="code_command hotspot=13-64" subs="quotes"]
----
...
[role="code_command hotspot=12-56"  subs="quotes"]
----
...
In the [hotspot=23-25 file=2]`setup()` step of the `JwtTest` test, an authorization header is created with the credentials of a test user who has the `TESTUSER` name and the `user` role.
...
To see whether the tests detect a failure, remove the authorization header generation in the [hotspot=24-26 file=1]`setup()` method of the `InventoryEndpointTest.java` file.
...
You learned how to use MicroProfile JWT to validate JWTs and authorize users to secure your microservices.

...
The MicroProfile JWT feature, [hotspot=2-3]`mpJwt`, has been added as a dependency in your `start/backendServices/pom.xml` file.
...
[role="code_command hotspot=1-23"]
----
Create a configuration file `backendServices/src/main/liberty/config/server.xml`.
----
...
The [hotspot=21-22]`<mpJwt>` element is required to configure the injection of the caller's JWT.

|===
| [hotspot=21-22]`issuer` | Issues security tokens and must match the `iss` claim in the JWT.
| [hotspot=21-22]`audiences` | Identifies the recipients and must match the `aud` claim in the JWT.
| [hotspot=21-22]`keyName` | Specifies the key alias name to locate the public key for JWT signature validation and must be in the keystore in the SSL configuration.
|===
...
=== Securing the system service
*Option A: Copy all the following code*
[role="code_command hotspot=1-20"]
----
Replace the file `backendServices/src/main/java/io/openliberty/guides/system/SystemResource.java` with roles-based access control.
----
*Option B: Add this part of the code in*
[role="code_command hotspot=14-15"]
----
Add roles-based access control to the `backendServices/src/main/java/io/openliberty/guides/system/SystemResource.java` file.
----
SystemResource.java
...
Role names that are used in the [hotspot=14-15]`@RolesAllowed` annotation are mapped to group names in the `groups` claim of the JWT, which results in an authorization decision wherever the security constraint is applied.

The [hotspot=14-15]`@RolesAllowed({"admin", "user"})` annotation allows only authenticated users with the role of `admin` or `user` to access the `system` service.
...
=== Securing the inventory service
*Same as above, we can use the other option*
[role="code_command hotspot=1-52"]
----
Replace the file `backendServices/src/main/java/io/openliberty/guides/inventory/InventoryResource.java`  with roles-based access control.
----
...
Similarly, for each specific service, the [hotspot=46-47]`@RolesAllowed` annotation sets which roles are allowed to access it. Therefore, the inventory service is properly secured, as well.

In addition, the [hotspot=28-44]`getPropertiesForHost()` method gets the HTTP Authorization header, which contains the corresponding security token, from the HTTP request caller. The method uses this authorization header to retrieve information from the `system` service.

The final addition of the [hotspot=32-33]`manager.get(hostname, authHeader)` code adds the authentication header to the client that sends the HTTP `GET` request to the `system` service.
...
[role="code_command hotspot=1-38"]
----
Create the `backendServices/src/main/java/io/openliberty/guides/inventory/client/SecureSystemClient.java` file.
----
...
However, the `SecureSystemClient` class overrides the [hotspot=19-24]`buildClientBuilder()` method by adding the authorization header to the returned builder, [hotspot=32-33]`return builder.header(HttpHeaders.AUTHORIZATION, authHeader)`.
...
[role="code_command hotspot=1-54"]
----
...
The [hotspot=27-29]`getJwtUsername()` function retrieves the user name from the injected `JsonWebToken jwtPrincipal` token by calling the `getName()` method.

The [hotspot=34-42]`getJwtGroups()` function retrieves the user groups information from the JSON Web Token.
The [hotspot=35-36]`SecurityContext.getUserPrincipal()` method returns an object in the `java.security.Principal` type. This object is also an instance of the `org.eclipse.microprofile.jwt.JsonWebToken` type, which has the `getGroups()` method.

The [hotspot=47-53]`getCustomClaim()` function retrieves the custom claim from the token if the authenticated user has the [hotspot=44-45]`admin` role. Otherwise, it returns a [hotspot=52-53]`Status.FORBIDDEN` message.
The [hotspot=48-49]`SecurityContext.isUserInRole(String)` method returns a `true` value for any role that appears in the MicroProfile JWT `groups` claim because role names are mapped to group names in the claim.
The [hotspot=48-49]`jwtPrincipal.getClaim("customClaim")` method retrieves the value of the custom claim by its name. This method is useful when you want to validate information about a custom claim from the security token.
...
[role="code_command hotspot=14-62"]
----
Create the `backendServices/src/test/java/it/io/openliberty/guides/jwt/SystemEndpointTest.java` file:
----
...
[role="code_command hotspot=14-77"]
----
Create the `backendServices/src/test/java/it/io/openliberty/guides/jwt/InventoryEndpointTest.java` file:
----
...
[role="code_command hotspot=1-56"]
----
Create the `backendServices/src/test/java/it/io/openliberty/guides/jwt/JwtTest.java` file:
----
...
The [hotspot=33-45]`testJWTGetName()` test accesses the `https://localhost:5051/inventory/jwt/username` endpoint to get the username from the JWT token and to verify
whether the user name is the same as the `TESTUSER` user name.

The [hotspot=47-54]`testJWTGetCustomClaim()` test accesses the `https://localhost:5051/inventory/jwt/customClaim` endpoint. Because this service is secured and only accessible to users who have the `admin` role,

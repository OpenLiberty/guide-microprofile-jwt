...
(expiration time), and `name`. Custom claims can be added to the token to include any other
...
of the JWT, such as the user's role, can be trusted and will be used to determine
...
You can try accessing these services without a JWT by entering the inventoryURL,
...
In this class, the `@RolesAllowed` annotation is limited to `admin` meaning only authenticated users with the role of `admin` are able to access the `inventory` service
...
The `inventory` service will now propagate the Authorization header, containing the JWT, to the `system` service. 

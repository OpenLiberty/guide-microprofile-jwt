...
`org.eclipse.microprofile.jwt.JsonWebToken` class. To see a full list of
accessors for different claims, including how to access custom claims added when
the JWT was built, check out the MP-JWT documentation [add link].
...
Once you log in, you can see some basic information retrieved from the backend
services. Remember that if you log in as a `user` without the `admin` role, you
cannot see the inventory size because you do not have permission.

As before, you can enter the inventory URL,
https://localhost:9443/inventory/systems[https://localhost:9443/inventory/systems],
or the system URL,
https://localhost:8443/system/properties[https://localhost:8443/system/properties]
directly into your browser to check that they are secured against users making a
request without a valid JWT included.
...
You will be able to see the different claims in the payload of your token:

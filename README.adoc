...
In the `start/backendServices/src/main/liberty/config` directory, create a configuration file named `server.xml`.
...
|===
| `issuer` | Issues security tokens and should match the `iss` claim in the JWT.
| `audiences` | Identifies the recipients and should match the `aud` claim in the JWT.
| `keyName` | Specifies the JWT signature validation key and should match the key in the truststore in the SSL configuration.
|===
...
Open the `start/backendServices/src/main/java/io/openliberty/guides/system/SystemResource.java` file and modify it to match the following code:
...
Role names used in the `@RolesAllowed` annotation are mapped to group names in the `groups` claim of the JWT, which results in an authorization decision wherever the security constraint is applied.
...
In the `start/backendServices/src/main/java/io/openliberty/guides/inventory/` directory, open the `InventoryResource.java` file  and add roles-based access control as shown in the following command:
...
However, after you secure the system service with token-based authentication, you need to add security tokens while sending HTTP requests through the client.
...
The `getJwtUsername()` feature retrieves the user name from the injected `JsonWebToken jwtPrincipal` principal by calling the `getName()` claim.

The `getGroups()` feature retrieves the user groups information from the JWT from the SecurityContext.
The `SecurityContext.getUserPrincipal()` feature returns an object in the `java.security.Principal` type. This object is also an instance of the `org.eclipse.microprofile.jwt.JsonWebToken` type.

The `getCustomClaim()` claim retrieves the custom claim from the token if the authenticated user has the `admin` role. Otherwise, it returns a `Status.FORBIDDEN` message.
The `SecurityContext.isUserInRole(String)` feature returns a `true` value for any role that appears in the MicroProfile JWT `groups` claim because role names are mapped to group names in the claim.
The `jwtPrincipal.getClaim("customClaim")` feature retrieves the value of the custom claim by its name. This feature is useful when you want to validate information about a custom claim from the security token.
...
After the Open Liberty application server starts, you can log in to the system with the simple front end. The entire front end code is provided for you to test if you can retrieve information from the back end services by using a valid security token.
...
Once you log in, you can see some basic information about your current session, including the name of the user who is currently logged in, the role of the user, your current operating system, the size of your inventory service, and the JWT that is used for authorization.
...
Next, create a `start/backendServices/src/test/java/it/io/openliberty/guides/jwt/InventoryEndpointTest.java` file:
...
This authorization header is added when the test client sends a GET request to a secure service to retrieve information.
...
After assuring that the response code is correct, each test tries to get the content from the designated endpoint and compares the result with its expected value.

Create a `start/backendServices/src/test/java/it/io/openliberty/guides/jwt/JwtTest.java` file:
...
The authorization header is added when the test client sends a GET request to a secure JWT service.
...
if the user name is the same as the `TESTUSER` user name.

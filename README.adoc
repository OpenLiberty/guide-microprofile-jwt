...
You will be working with 2 services, a frontend service and a backend `system`
service. The frontend service logs a user in, and makes authorized requests to
the secured `system` service using JWTs.
...
frontend has requeted from the `system` service. If you log in as an `admin`
user, the current OS will be shown. If you log in as a `user`, you will instead
see the message `You are not authorized to access this system property`, as the
`user` role does not have sufficient privileges.

Additionally, the `groups` claim of the JWT are read by the system service nad
...
change. Navigate to the `frontend` and run the following goal to start the
...
[source, java, linenums, role='code_column hide_tags=copyright,getJwtUsername,getJwtGroups']
----
...
The `@RolesAllowed` annotation automatically reads from the `groups` claim of
the JWT to make an authorization decision. If you wish to manually access the
claims of the JWT, perhaps to provide more granual access control, MicroProfile
JWT provides you with the [hotspot=claim file=0]`@Claim` annotation. In this
case, the `"groups"` claim is injected into the [hotspot=rolesArray
file=0]`roles` JSON array. These roles parsed from the `"groups"` claim of the
JWT are then exposed back to the frontend at the [hotspot=rolesEndpoint
file=0]`/jwtRoles` endpoint. To reade more about different claims and ways to access them, check out the 
https://github.com/eclipse/microprofile-jwt-auth/blob/master/spec/src/main/asciidoc/interoperability.asciidoc[MP-JWT
documentation^].

// =================================================================================================
// Creating a client to access the secure server
// =================================================================================================
...
As stated previously, because the `system` service is secured, requests made to
it must include a valid JWT in the `Authorization` header. The
[hotspot=headerParam1 hotspot=headerParam1 hotspot=headerParam1
file=0]`@HeaderParam` annotation is used to specify that the value of the
`String authorization` parameter of this method is to be sent in the `Authorization`
header of requests made through the client. 
...
of the `SystemClient` class you created, which makes a request to the `system`
service for data. Passed in as a paramter to these methods is
[hotspot=authHeader1 hotspot=authHeader2 hotspot=authHeader3
file=1]`authHeader`, or the authentication header, which is the JWT with `Bearer
` prepended to it. The JWT is retreived from the session attributes using the
[hotspot=getJwt file=1]`getJwt()` method. When the authentication header is
passed into the `[hotspot=restClient file=1]`defaultRestClient` methods, it is
added to the `Authorization` header of the associated requests made to the
`system` service.

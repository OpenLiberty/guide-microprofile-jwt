...
included in requests made to it. The claims in the JWT are used to determine what
...
The [hotspot=issuer file=0]`mp.jwt.verify.issuer` config property specifies what the expected value of
...
Next, add the MicroProfile JWT feature to the server configuration file for
the `system` service. 

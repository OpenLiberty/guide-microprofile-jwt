...
file=0]`admin` role, one with a JWT with the just the [hotspot=userRequest1
hotspot=userRequest2 hotspot=userRequest3 file=0]`user` role, and a third with
[hotspot=nojwtRequest1 hotspot=nojwtRequest1 hotspot=nojwtRequest1 file=0]`no
JWT` at all. The responses of these requests are checked for correctness based

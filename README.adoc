...
In this case, the application will be using JWTs to authenticate a user, allowing them to make
...
whenever you save a new change. Navigate to the `start/frontend` directory and
run the following goal to start the `frontend` service in dev mode:
...
hotspot=authHeader2 hotspot=authHeader3 file=1]`authHeader` is a string that
consists of the JWT with `Bearer` prepended to it. The [hotspot=authHeader1
hotspot=authHeader2 hotspot=authHeader3 file=1]`authHeader` is included in the

...
Security tokens are a common way to propagate the security state from clients to services or from services to services. The main security protocols are based on security tokens, such as OAuth2, OpenID Connect, SAML, WS-Trust, WS-Federation, and others. While some of these standards are closer to identity federation, they share a common concept regarding security tokens and token-based authentication.
...
Break down the annotations:

- `@Path("jwt")` states the REST request path to access this class, which in this case is JWT.
...
The `SecuredSystemClient` class inherits methods from the `SystemClient` class by sending the HTTP GET request to the system service to retrieve system properties.
...
Add the `SystemResource.java` class in the `finish/backendServices/src/main/java/io/openliberty/guides/system/SystemResource.java` directory.
...
Create a `LoginBean.java` class in the `/start/frontendUI/src/main/java/io/openliberty/guides/ui/` directory to handle login requests to the back end. Create the JWT token required for authentication.

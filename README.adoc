...
You will add MP-JWT to validate security tokens in the `System` and `Inventory` microservices, and add a simple front end to handle the authentication.
...
The MicroProfile JWT feature (`microprofile-jwt-auth-api`) is added as a dependency in your `start/backendServices/pom.xml` file. This feature allows you to
...
You will write `EndpointWithJwtTest` and `JwtTest` classes to test the accessibility of different services.

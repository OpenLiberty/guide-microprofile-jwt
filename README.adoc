...
You will learn how to control user and role access to microservices using MicroProfile JWT. The great benefit from adding JWT security to your microservices is that it allows certain groups of users access to specific functions.
...
The `doLogIn()` method authenticates users who are pre-defined in the front-end `server.xml` file and creates a user object and a JWT token. It will then create an http session used for continued access to the application.
...
The `getOs()` method returns a String of the operating system for a session from the back-end by doing a GET request with the auth header containing the JSON Web Token.

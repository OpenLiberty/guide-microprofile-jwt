...
The user will sign into the `frontend` service using a username and a password, at
which point a JWT will be created. The frontend will then make requests to the
backend `system` service with the JWT included in the requests. The `system`
service, which is secured, will verify the JWT included with the request to
ensure that the request did in fact come from the authorized frontend. After the
JWT has been validated, the information in the claims, such as the
user's role, can be trusted and will be used to determine which system
properties the user has access to.

To learn more about Json Web Tokens, check out the
https://jwt.io/introduction/[JWT.io website] and to learn more about JWT
can be used for user authentication and authorization, check out Open Liberty
documentation on https://openliberty.io/docs/latest/single-sign-on.html[Single
Sign-on].

...
For microservices, Token Based Authentication mechanism offers a very lightweight way for security controls, and security tokens propagate user identities across different services. JSON Web Token (JWT) is becoming the most common token format to protect services because it follows a well defined and known standard.

MicroProfile JSON Web Token (MP-JWT) standard defines the required format of JWT for authentication and authorization, maps its token claims to various Java EE container APIs, and makes the set of claims available through get style accessors.

You will add MP-JWT to validate security tokens in the `System` and `Inventory` microservices.  You will use Token Based Authentication mechanism to authenticate, authorize and verify user identities based on a security token.

In addition, you will learn how to verify token claims through get style accessors using MicroProfile JWT.
...
Point your browser to the front-end web application endpoint: `http://localhost:9090/application.jsf`. From here, you can log in to the system with the form-based login provided.
...
// =================================================================================================
// Mapping MP-JWT with Java EE Container APIs
// =================================================================================================

=== Mapping MP-JWT with Java EE Container APIs

To demonstrate how MP-JWT retrieves confidential information and validates custom claims from the security token, add an additional service that uses JAX-RS security related methods. The JWT service also illustrates how Java EE container APIs map to MP-JWT features.

Create a `start/backendServices/src/main/java/io/openliberty/guides/inventory/JwtResource.java` file to enable a service that retrieves information from the JSON Web Token.
...
MicroProfile JSON Web Token (MP-JWT) maps its token claims to JAX-RS security related methods, and makes the set of claims available through get style accessors.
...
Once you log in, you can see some basic information about your current session, including the user name of the user who is currently logged in, your role[s], your current operating system, the size of your inventory service, and the JSON Web Token (JWT) that is  used for authorization.

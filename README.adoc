...
The [hotspot=33-45 file=2]`testJWTGetName()` test accesses the `https://localhost:5051/inventory/jwt/username[https://localhost:5051/inventory/jwt/username^]` endpoint to get the username from the JWT token and to verify
whether the username is the same as the `TESTUSER` user name.

The [hotspot=47-54 file=2]`testJWTGetCustomClaim()` test accesses the `https://localhost:5051/inventory/jwt/customClaim[https://localhost:5051/inventory/jwt/customClaim^]` endpoint. Because this service is secured and only accessible to users who have the `admin` role,

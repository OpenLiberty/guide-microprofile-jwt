...
part being seperated by a dot (`.`). An encoded token typically looks like
the following:
...
starts that starts the frontend module in the background.
...
In a new tab of your terminal, navigate to the `system` directory and run the following goal:
...
https://localhost:9090/inventory/systems[https://localhost:9090/inventory/systems],
or the system URL,
https://localhost:9090/system/properties[https://localhost:9090/system/properties]
...
the `iss` claim on an incoming token is. Incoming tokens with a different issuer
claim are not considered valid.
...
This class has roles-based access control. Role names that are used in the
...
When the `inventory` service requests properties from the `system` service, there must be 
a JWT included in the `Authorization` header of that request. 

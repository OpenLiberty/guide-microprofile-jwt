...
microservice. You will use a token-based authentication mechanism to
authenticate, authorize, and verify user identities based on security tokens.
...
image::assets/JWT_diagram.png[JWT frontend system, align="center"]
...
(expiration time), and `groups` (the groups or roles of the user). Custom claims
can be added to the token to include any other information you would like to
transmit.
...
In this case, we will be using JWTs to authenticate a user, allowing them to make
authorized requests to the backend `system` service. 

The user will sign into the `frontend` service using a username and password,
...
frontend's public key to verify the signature of the JWT. This ensures that the
...
To try out the application, first navigate to the `finish/frontend` directory.
...
Open another command-line session, navigate to the `finish/system` directory and run the following Maven goal:
...
change. Navigate to the `start/frontend` directory and run the following goal to start the
...
Open another command-line session, navigate to the `start/system` directory
...
Create a secure service that provides system properties to the frontend `service`.
...
The application bean provides the methods used by the web application to populate the
table in the UI. The web application code has already been given to you. The methods
...
their associated method of the [hotspot=systemClient file=0]`SystemClient` class
with the [hotspot=authHeader1 hotspot=authHeader2 hotspot=authHeader3
file=1]`authHeader`, passed in as a parameter. The [hotspot=authHeader1
hotspot=authHeader2 hotspot=authHeader3 file=1]`authHeader` is the JWT with
`Bearer` prepended to it and is added to the `Authorization` header of the
requests made by the [hotspot=restClient file=1]`defaultRestClient`.

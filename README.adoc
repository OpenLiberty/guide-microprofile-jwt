...
(expiration time), `name`, or any other information you would like to transmit.
...
at which point a JWT will be created and signed using a private key stored in
the frontend. The frontend will then make requests to the backend services with
the JWT included in the `Authorization` header of the request. The backend
services, which are secured, will examine the JWT included with the request and
use the frontends public key to verify the signature of the JWT to ensure that
the request did in fact come from the authorized frontend and wasn't changed
along the way. After a JWT has been validated, the information stored the claims
of the JWT, such as the user role, can be trusted and will be used to determine
which services the requesting user has access to in the backend.
...
To try out the application, first navigate to the `finish` directory. Next,
...
to be in the request. If you log in as an `admin` user, the current inventory size
appears and can add systems to this inventory using the interface below it. If you
log in as a `user`, you will instead see the message, `You are not authorized to
access the inventory service`, which means you cannot access the `inventory`
...
because you didn't supply a valid JWT with the request. 
...
directory run the following goal to start the `inventory` service in `dev` mode:
...
In a new tab of your terminal, navigate to the `system` directory and run the
...
The `mp.jwt.verify.issuer` config property specifies what the expected value of
the `iss` (issuer) claim on an incoming token is. Incoming tokens with an issuer
claim different from this expected value are not considered valid.

The `mp.jwt.verify.publickey.location` config properties specifies the location
of the public key to be used to validate the an incoming token's signature. In
this case, the `frontend` service, where our JWT is built, provides an endpoint
to retrieve the public signing key.


// =================================================================================================
// Securing the inventory and system services
// =================================================================================================

== Securing the inventory and system services

Next, create the `SystemResource` class.

[role="code_command hotspot", subs="quotes"]
----
#Create the `SystemResource` class.#
`inventory/src/main/java/io/openliberty/guides/system/SystemResource.java`
----

This class has role-based access control. Role names that are used in the
[hotspot=RolesAllowed]`@RolesAllowed` annotation are mapped to group names in
...
Create a secured `InventoryResource` class.
...
Similarly, for each specific service, the `@RolesAllowed` annotation sets which
roles are allowed to access it. Therefore, the `inventory` service is properly
secured as well.
...
The `@RegisterClientHeaders` annotation instructs the client to propagate the
...
In the `org.eclipse.microprofile.rest.client.propagateHeaders` MicroProfile
Config property for the inventory service, specify the `Authorization` header.
...
the token and makes an authorization decision.
...
Create a new service that uses these accessors to read the `name` and `groups`
claim of the incoming JWT and show that this information was able to be retrived
...
In this case you are using the `getName()` and `getGroups()` accessors of the
`org.eclipse.microprofile.jwt.JsonWebToken` class. To see a full list of accessors for different claims, including how to access custom claims added when the JWT was built, check out the MP-JWT documentation [add link].
...
Use the following endpoint to get to the login page of your application:

* http://localhost:9090/login.jsf[http://localhost:9090/login.jsf^]
...
Once you log in, you can see some basic information retrieved from the backend services.
...
As before, you can enter the inventory URL,
https://localhost:9443/inventory/systems[https://localhost:9443/inventory/systems],
or the system URL,
https://localhost:8443/system/properties[https://localhost:8443/system/properties]
directly into your browser to check that they are secured against users making request without a valid JWT included.

Once you are logged, you should see your JWT displayed in the bottom row labeled `JSON Web Token`.

To see the information that this JWT holds, you can enter it into the JWT.io website[https://jwt.io], which includes a token reader.

You will be able to see the different claims in the payload of our token:

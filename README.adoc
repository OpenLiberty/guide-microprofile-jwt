...
|===
| `issuer` | Issues security tokens, should match the `iss` claim in the JWT
...
When the Open Liberty application server has started, you can log in to the system with the simple front end. The entire front end code is provided to test if you can retrieve information from the back end services using a valid security token.

You cannot directly visit a back end URL from a browser since there is no valid tokens in the authorization header when you send HTTP requests via a browser. You have to use the provided front end to create a web client to send HTTP GET requests with valid security tokens.
...
Once you log in, you can see some basic information about your current session, including the name of the user who is currently logged in, the role[s] of the user, your current operating system, the size of your inventory service, and the JWT that is  used for authorization.

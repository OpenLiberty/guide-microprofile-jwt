...
The [hotspot=2-3]`mpJwt` feature adds the libraries that are required for MicroProfile JWT implementation.
...
Similarly, for each specific service, the [hotspot=47]`@RolesAllowed` annotation sets which roles are allowed to access it. Therefore, the inventory service is properly secured, as well.
...
However, the `SecureSystemClient` class overrides the [hotspot=20-24]`buildClientBuilder()` method by adding the authorization header to the returned builder, [hotspot=32]`return builder.header(HttpHeaders.AUTHORIZATION, authHeader)`.
...
The [hotspot=27-29]`getJwtUsername()` function retrieves the user name from the injected `JsonWebToken jwtPrincipal` token by calling the [hotspot=28]`getName()` method.

The [hotspot=34-42]`getJwtGroups()` function retrieves the user groups information from the JSON Web Token.
The [hotspot=36]`SecurityContext.getUserPrincipal()` method returns an object in the `java.security.Principal` type. This object is also an instance of the `org.eclipse.microprofile.jwt.JsonWebToken` type, which has the [hotspot=39]`getGroups()` method.

The [hotspot=47-53]`getCustomClaim()` function retrieves the custom claim from the token if the authenticated user has the [hotspot=45]`admin` role. Otherwise, it returns a [hotspot=52]`Status.FORBIDDEN` message.
...
In the [hotspot=22]`setup()` step before test cases, an authorization header is created with the credentials of a test user who has the `TESTUSER` name and the `admin` role.
...
In the [hotspot=22]`setup()` step of the `JwtTest` test, an authorization header is created with the credentials of a test user who has the `TESTUSER` name and the `user` role.
...
To see whether the tests detect a failure, remove the authorization header generation in the [hotspot=22]`setup()` method of the `InventoryEndpointTest.java` file.

...
The [hotspot=10-32]`SecureSystemClient` class inherits methods from the `SystemClient` class. These methods hand 
the HTTP `GET` request to the system service to retrieve system properties. However, the [hotspot=10-32]`SecureSystemClient` 
...
header to the client request, the [hotspot=10-32]`SecureSystemClient` class can access services that are secured.

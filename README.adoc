...
You will be redirected to a page that displays some information. The frontend
requests the inventory size and current OS from the `inventory` service. To get
the current OS, the `inventory` make a request to the `system` service. Both of
the `inventory` and `system` services are secured and require a valid JWT to be in the request's
header. 

If you log in as an `admin` user, the current inventory size appears and
can add systems to this inventory using the interface beneath the bottom row. If
you log in as a `user`, you will instead see the message, `You are not
authorized to access the inventory service`, as the role does not have
sufficient privileges. 

Additionally, the `upn` (unique principal name, or username) and `groups` claim
of the JWT are read by the inventory service and are requested by the frontend
to be displayed.
...
As stated previously, because the `system` service is secure, requests made to
it must include a JWT in the header. In this case, the `inventory` receives
requests with a JWT in the `Authorization` header, so you must propagate that
header to the `system` service what making requests to it.

The [hotspot=registerClientHeaders file=2]`@RegisterClientHeaders` annotation
instructs the client to propagate the headers specified in the MicroProfile
Config property [hotspot=propagateHeaders
file=3]`org.eclipse.microprofile.rest.client.propagateHeaders`.
...
In the previous section you implemented role-based access control with the
...
Update the `inventory` service to use these provided accessors to read the `upn` (unique principal name, or user name) and `groups`
...
You can also see the values of the `upn` and `groups` claims in the top two

...
The payload contains the claims of a token. Claims are statements about the
...
and verifies that the sender of the JWT are who they claim to be.
...
application. Give it a try before you proceed.

To try out the application, first navigate to the `finish/frontend` directory
and run the following Maven goal to build the application and deploy it to Open
Liberty:
...
attributed by the provided [hotspot file=2]`LoginBean` class. When the

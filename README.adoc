JSON Web Token// Copyright (c) 2018 IBM Corporation and others.
...
You will add JWT authentication and authorization to the microservices provided: `SystemResource` and `InventoryResource` while we provide you with a simple front end to handle the authentication.
...
Point your browser to front-end web application endpoint: `http://localhost:9090/system.jsf`. From here you have the ability to log into the system using the form based login provided.
Use the admin username and password `Username: bob` `Password: pwd` then click the log in button. You should now be redirected to a another page with that will display the logged in User, the current OS of your localhost and the web token that is being used by the front-end to access the data.
...
Role names used in ``@RolesAllowed` have been mapped to group names in the MP-JWT "groups" claim, result in an allowing authorization decision wherever the security constraint has been applied.

The `getJwtUserName()` method retrieves the user name from the injected `JsonWebToken jwtPrincipal`.

The `getGroups(@Context SecurityContext securityContext)` method gets the user groups information from the Json Web Token from the SecurityContext.
...
The `getStatus(@Context SecurityContext securityContext)` method is used to get the custom claim from the provided Json Web Token if the authenticated user has the role of `admin`. Otherwise, it returns `Status.FORBIDDEN`.
...
The following code `String authHeader = httpHeaders.getRequestHeaders().getFirst(HttpHeaders.AUTHORIZATION);` retrieves the authorization header from the GET request.
...
We have provided you with a very simple front-end web application to allow you to log into the application with pre defined users that have different roles such as `admin` and `user`.
...
An `User` class has been provided for you in `/start/frontendUI/src/main/java/io/openliberty/guides/ui/User.java`.

This is an user model that contains information regarding a user. It has `get` and `set` methods for the username, password and role.
...
The `doLogIn()` method authenticates users who are pre-defined in the front-end `server.xml` file and creates a user object and a JWT token. It will then create a http session used for continued access to the application.

The `buildJWT(String userName, String role)` method is used to create a JWT token with provide user information.
...
The `getOs()` method returns a String of the operating system for the session from the back-end by doing a GET request with the auth header containing the JSON Web Token.

The `getInvSize()` method returns the size of the inventory list for a given user from the session.

The `getJwt()` method returns the JSON Web Token for the current session.

The `getUserName()` method returns the username of the user currently logged in from the session.

The `getUserRole()` method returns the role of the currently logged in user.
...
Create a class `server.xml` in `/start/frontendUI/src/main/liberty/config` to pre-define users with their names, passwords and roles.
